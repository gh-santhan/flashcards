...
function bindEditorActions(){
  ...
  if (tbl) {
    // ensure only one table handler
    if (tbl._edHandler) tbl.removeEventListener('click', tbl._edHandler);
  ...
  // Bulk delete modal (idempotent via `on` helper)
  on('btnBulkDelete','click', ()=>{
    const m = $('bulkDeleteModal');
    if (m){ m.style.display='flex'; setupBulkDeleteUI(); }
  });
  on('bdClose','click', ()=>{
    const m = $('bulkDeleteModal');
    if (m) m.style.display='none';
